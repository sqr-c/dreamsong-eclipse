...
{
	{
		...
		4, 3,
		//MAP SEEK:		.\translation\m5\0000.smd		arena 0
		95720,
		//MAP SEEK:		.\translation\m4\0000.smd		arena wait entrance 0
		91792,
	...
	},
	{
		...
		//load in arena wait position
		9, 5,
		//MAP SEEK:		.\translation\m9\0000.smd		arena 1
		156040,
		//MAP SEEK:		.\translation\m10\0000.smd		arena wait entrance 1
		51584,
...
void update_arena()
{
	...
	if (map.npca.size == 0)
	{
		...
		map.npca.size++;
		map.npca.arr[map.npca.size - 1].ch.lifea = MAX_LIFE;
...
void init_arena()
{
	...
	for (int i = 0; i < sizeof(map.adj) / sizeof(long); i++)
		map.adj[i] = -1;
...
char tile_load_arena(m_map* m, int loadidx)
{
	...
	m->loads.arr[0].seek = arenasxy[0].seek;
	m->loads.arr[0].ox = arenasxy[0].x - defr;
	m->loads.arr[0].oy = arenasxy[0].y - defr;
...
void trs_ent_arena()
{
	...
	for (int i = 0; i < map.npca.size; i++)
	{
		...
		map.npca.arr[i].ch.state = 1;
		set_state(&map.npca.arr[i].ch, 0, 0);
	}
