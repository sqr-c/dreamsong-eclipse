...
{
	short mov = (SDL_GetTicks() / (int)(32 * 256));
...
{
	return get_element_spraa(&(map->tile), x + y * map->width);
}
...
m_map readmapp(const char* path, long offset)
{
	...
	//adjacents
	fread(mapbuf.adj, sizeof(long), MAP_ADJ_LEN, mapfp);
...
void writemapp(m_map* map, const char* path)
{
	...
	//adjacents
	fwrite(map->adj, sizeof(long), MAP_ADJ_LEN, mapfp);
}
...
void writemapex(m_map* map, const char* path, const char* mode)
{
	...
	//adjacents
	fwrite(map->adj, sizeof(long), MAP_ADJ_LEN, fp);
...
inline void writenpca(n_npc_a* c, const char* path, long npcoffset)
{
...
char load_mapload(int i)
{
	...
	{
		if ((flag = tile_load_map_spec(&map, i)) == 1)
			return 0;
	}
	...
	else
	{
		...
		//testing bug fix, normally out of else scope
		push_back_mapload(
			&map.loads,
...
char collision_map_char(const ch_char* ch)
{
	for (int i = 1; i < 4; i++)
	{
		...
		if (x < 0 + (defr * statusr)
			|| y < 0 + (defr * statusr)
			|| x >= map.arr[0].width - (defr - 1) * statusr
			|| y >= map.arr[0].height - (defr - 1) * statusr)
			return 1;

		s_sprite* s;

		s = &map.arr[1].tile.arr[p2idx(x, y, map.arr[0].width)].arr[0];
		if (s->flag >= 1)
			return s->flag;
		s = &map.arr[0].tile.arr[p2idx(x, y, map.arr[0].width)].arr[0];
		if (s->id == -1)
			return 1;
	}
...
{
	if (map->ox > x || map->oy > y || map->width - 1 + map->ox < x || map->height - 1 + map->oy < y)
		return 0;

	s_sprite_a* sa = get_tile(map, x - map->ox, y - map->oy);
	for (int i = 0; i < sa->size; i++)
		if (sa->arr[i].flag >= 1 && sa->arr[i].flag <= 3)
			return sa->arr[i].flag;
...
bt_breaktile_h create_breaktileh_auto(m_map* map)
{
	...
	{
		buf.arr[i].breakmb = buf.arr[i].breakma = (map->arr[1].tile.arr[i].arr[0].flag == 2) * 8;
...
{
	//false, this code is likely to contain a bug
	//range of w*h includes the size. (w-1)(h-1) will not include the size, thus averting a buffer overflow
	{
		int w = m->arr[0].width, h = m->arr[0].height;

		for (int y = 0; y < h; y++)
			for (int x = 0; x < w; x++)
				buf->breaks.arr[p2idx(x + xo, y + yo, w)]
				= m->breaks.arr[p2idx(x, y, w)];
	}
			...
			for (int ii = 0; ii < 2; ii++)
				for (int i = 0; i < map.cache.size; i++)
				{
					...
					if (
						...
						||
						(flag != SPRITE_FLAG_CACHE && flag != SPRITE_FLAG_DROP)
						)
...
void collision_full()
{
	char colbuf = collision_map_char(&ch);
	if (debug_ch_collison &&
		(colbuf || collision_npca_char(&(map.npca), &ch)))
	{
		if (colbuf == SPRITE_FLAG_BLOCK)
			goto lbl_block;

		else if (colbuf == SPRITE_FLAG_STCHNG)
			tile_set_map_state();

		else if (SPRITE_FLAG_DROP == colbuf)
		{
			s_sprite* s;
		...
		else if (SPRITE_FLAG_TFN == colbuf)
		{
			for (int i = 0; i < map.tfn.size; i++)
			{
		...
		else if (SPRITE_FLAG_POPLSTACK == colbuf)
		{
		...
		else if (SPRITE_FLAG_POPDSTSTACK == colbuf)
		{
			int mi = 0;
			int md = -1;
			for (int i = 0; i < map.loads.size; i++)
