...
#define ELEM_SCREEN_WIDTH	4
		...
		blit_border(BORD_OFFSET, bx, by, ba + bx, by + ba + 1);

		if (blit_tick_elem_cache && menu_sel_idx_1 == i && seltik) continue;
		else if (!l)
		{
			blit_elem(ELEM_OFFSET + ELEM_CACHE_MAX * cache[i].id,
			...
			{
				int o =
					!((int)(((1 + cache[i].spra) / ELEM_SPR_BLOCK)) > (ii - 2));
				quadblit(&(sprdata[FLAT_OFFSET + o]), (ii + bx) * 16, (by + 8 + .5f) * 16);
			}
		}
	}
...
#define BORD_TOKEN_MARK (BORD_OFFSET + 14)
	...
	for (int i = 0; i < 16; i++)
	{
		int offs = BORD_OFFSET + 12 + ((i) >= (sprb / SPR_BLOCK));

		tilerect = (SDL_Rect){ sprdata[offs].OX, sprdata[offs].OY, sprdata[offs].OW, sprdata[offs].OH };
		surfacerect = (SDL_Rect){ 16 * (i + 5 + 8) - sprdata[offs].CX - sprdata[BORD_TOKEN_MARK].CX, 16 * (+26) - sprdata[offs].CY * 0, sprdata[offs].OW, sprdata[offs].OH };
		SDL_RenderCopy(renderer, tile, &tilerect, &surfacerect);
	}
	...
	for (int i = 0; i < 6; i++)
	{
		s_spritedata* at = &(sprdata[
			pm(((token / (int)pow(10, (6 - 1 - i))) % 10) - 1, (TEXT_0 - TEXT_1 + 1)) + TEXT_1
		]);
...
inline void blit_elem_c(int id)
{
	...
	blit_border(BORD_OFFSET, xo / 16, (yo / 16), (xo / 16) + (128 / 16 - 1), (yo / 16) + 128 / 16 - 1);
	blit_elem(id, xo, yo);
}
...
#define BORD_COLOR	6
	...
	blit_border(BORD_OFFSET + BORD_COLOR, 0, yy / 16, o + 32, o + yy / 16);

	blit_column(BORD_OFFSET + BORD_COLOR, o, yy / 16 + 1, -1 + o + yy / 16, tileh / 2);
	blit_column(BORD_OFFSET + BORD_COLOR, (SCREEN_WIDTH / 2) / 16, yy / 16 + 1, -1 + o + yy / 16, 0);
	blit_column(BORD_OFFSET + BORD_COLOR, o + (SCREEN_WIDTH / 2) / 16, yy / 16 + 1, -1 + o + yy / 16, SCREEN_HEIGHT / 2);

	if (p->e0 != -1) blit_elem(p->e0, 0, yy);
	...
	{
		{
			...
			tilerect.x = ELEM_AREA + sprdata[FLAT_OFFSET].OW * ((1.5f * (i / 2)) - .5f) - 4 / 2 + 1 + 10;
			tilerect.y = yy + (16 * o) / 2 + ((i % 2) * 1.5f - (1.0f / 2)) * sprdata[FLAT_OFFSET].OH - 8 - 4 / 2;

			SDL_RenderFillRect(renderer, &tilerect);
		}

		quadblit(&(sprdata[FLAT_OFFSET + (i >= p->sp0)]),
			ELEM_AREA + sprdata[FLAT_OFFSET].OW * ((1.5f * (i / 2)) - .5f)
			+ 1 + 10
			,
			yy + (16 * o) / 2 + ((i % 2) * 1.5f - (1.0f / 2)) * sprdata[FLAT_OFFSET].OH);
	}

	float oo = SCREEN_WIDTH / 2;

	if (p->e1 != -1) blit_elem(p->e1, oo, yy);
	...
	{
		{
			...
			tilerect.x = oo + ELEM_AREA + sprdata[FLAT_OFFSET].OW * ((1.5f * (i / 2)) - .5f) - 4 / 2 + 1 + 10;
			tilerect.y = yy + (16 * o) / 2 + ((i % 2) * 1.5f - (1.0f / 2)) * sprdata[FLAT_OFFSET].OH - 8 - 4 / 2;

			SDL_RenderFillRect(renderer, &tilerect);
		}

		quadblit(&(sprdata[FLAT_OFFSET + (i >= p->sp1)]),
			oo + ELEM_AREA + sprdata[FLAT_OFFSET].OW * ((1.5f * (i / 2)) - .5f)
			+ 1 + 10
			,
			yy + (16 * o) / 2 + ((i % 2) * 1.5f - (1.0f / 2)) * sprdata[FLAT_OFFSET].OH);
	}
...
//TODO: remake this function and the tiles where the sprites are stored
void blit_elem(int idx, int xp, int yp)
{
	int i = idx;
	for (int y = 0; y < ELEM_DIV; y++)
		for (int x = 0; x < ELEM_DIV; x++)
		{
			surfacerect = (SDL_Rect){ x * ELEM_W + xp, y * ELEM_H + yp, ELEM_W, ELEM_H };
			tilerect = (SDL_Rect){ sprdata[i].OX, sprdata[i].OY, ELEM_W, ELEM_H };
			SDL_RenderCopy(renderer, tile, &tilerect, &surfacerect);
...
#define TEXT_MARK (TEXT_OFFSET + 110 + 0*124 + 0*97)
	...
	for (int i = ls_ptr; i <= ls_max; i++)
	{
		...
		if (cache[i].mul > 1)
		{
			...
			for (int iii = 0; iii < 2; iii++)
			{
				at = &(
					sprdata
					[pm(((cache[i].mul / (int)pow(10, iii)) % 10) - 1, (TEXT_0 - TEXT_1 + 1)) + TEXT_1]);
