...
void init_en_npc(n_npc* buf, n_npc_min* npcm)
{
	...
	buf->ch.lifea = CH_MAX_LIFE;
	buf->ch.lifeb = CH_MAX_LIFE;
			...
			else
			{
				...
				if (at->ch.gcool <= tik && (tik0 != tik / castc))
				{
					...
					else
					{
						...
						//todo: ch act anim and ennpc act anim not the same. sync.
