...
{
	{
		//MAP SEEK:		./translation/wm1/0032.smd		arena 0
		3609188,
		0,
	},
	{
...
char npc_init_arena(n_npc* npc)
{
	...
	for (i = 0
		;
		0 == (mapstate_arenastate_table[i].id == loadstack.arr[loadstack.size - 1].load.seek)
		; i++);
...
char npc_init_market_0(n_npc* npc)
{
	...
	else if (npc->istate == 1)
	{
		...
		if (state == 0)
		{
			...
			gtidx = TEXT_TRADE0_IDX;

			set_text_txt(
				&gtexta[TEXT_TRADE0_IDX],
				&itexta[find->id],
				12,
				17,
...
char npc_swap_gem_dive(n_npc* npc)
{
	t_aidx = 0;
	t_bidx = 0;

	if (tfn_ch_state_dive(NULL))
	{
		char num_in_elem = 3;
		...
		{
			screen_mode = SCREEN_MODE_TEXT;
			gtidx = TEXT_TRADE1_IDX;
			set_trade1_text_num(num_in_elem, &itexta[ICAC_GEM_WATER_ID], &itexta[ICAC_C_DIVE_ID]);
			npcfn_state = 1;
		}
		else if (npcfn_state == 1)
		{
			screen_mode = SCREEN_MODE_DISP_TRADE;
			menu_sel_idx_2 = 0;
			...
			{
				trade_out_cache.id;
				trade_in_cache.id = ICAC_GEM_WATER_ID;
				trade_in_cache.mul = num_in_elem;

				trade_out_cache.id = ICAC_C_DIVE_ID;
				trade_out_cache.mul = 1;

				tradefn = &trade_item_item_s;
			}

			npcfn_state = 0;
		}
		else if (npcfn_state == 2)
		{
			screen_mode = SCREEN_MODE_NORMAL;
			npcfn_state = 0;
			return 0;
