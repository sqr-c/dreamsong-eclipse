...
#define SPRITE_DUST_ID_0	(SPRITE_FX_OFFSET + 3)
#define SPRITE_DUST_ID_1	(SPRITE_FX_OFFSET + 6)
#define SPRITE_DUST_ID_2	(SPRITE_FX_OFFSET + 9)
...
#define CD_RET_ACT_EX(fncd, gcd, acttik)										\
if (set_cooldown_act_ex(c, cd, c->ch == &ch, (fncd), (gcd), (acttik))) return 2;	\
else if (c->ch->spra <= 0) return 1;								\
...
#define CD_DEC_SPR_ACT_EX(fncd, fnspr, gcd, acttik)									\
CD_RET_ACT_EX((fncd), (gcd), (acttik))													\
dec_spra(c, (fnspr))											\
...
{
	//TODO: after switch .smd format, put flag for cast control in struct ch_char, replacing v
	if (c->state == CH_STATE_DIVE && cast[idx] != CAST_DIVE_ID) return;

	if (cast[idx] != -1)
	{
		if (2 == castfn[cast[idx]](v)) return;
	}
	else
	{
		c->acttick = c->gcool =
	...
	ch_set_state(c, c->rot, CH_STATE_ACTION);
...
char set_cooldown_act_ex(c_cast* c, int* castcd, int cidx, int t, int gcd, int acttik)
{
	if (castcd[cidx] > (int)tik) { return 1; }
	else castcd[cidx] = tik + t;
	...
	//act anim cd
	c->ch->acttick = tik + acttik;

	return 0;
}

#define set_cooldown(c, castcd, cidx, t)	set_cooldown_act_ex(c, castcd, cidx, t, t * 1.25f, 125)
...
char stdcast(c_cast* c, int a)
{
	if (c->ech != NULL)
	{
		...
		c->ech->lifea -= ((a / BREAKM_MAX) * CH_MAX_LIFE);
...
char cast1(c_cast* c)
{
	...
	stdcast(c, CAS_BREAK_ADD_FN_0(BREAKM_BLOCK * (2.0f / 4), esprl));
...
short dig_pre_loadstack;
char digmode = 0;
void dm_apply_dig()
{
	static seekxy seekxy;

	extern char cac_init_cache_flag;
	cac_init_cache_flag = digmode;
	...
	if (digmode)
	{
		...
		dig_pre_loadstack = loadstack.size;
	}
	else
	{
		//remove file
		...
		loadstack.size = dig_pre_loadstack;
...
char cast_dig(c_cast* c)
{
	...
	{
		char flag = map.arr[1].tile.arr[p2idx(c->ix, c->iy, map.arr[0].width)].arr[0].flag;
		if (flag == SPRITE_FLAG_BREAK || flag == SPRITE_FLAG_CACHE) return 2;
	}
...
char cast_dsp_illuminate(c_cast* c)
{
	...
	//add dust
	add_dust(c->ch->sprite.x, c->ch->sprite.y, 1.5f, SPRITE_DUST_ID_1, tik + 250 * 4);
...
char cast_gloomyhaze(c_cast* c)
{
	...
	//add dust
	{
		add_dust(c->ix, c->iy, 1.5f, SPRITE_DUST_ID_2, tik + 250 * 4);
		add_dust(c->ix, c->iy, 0.5f, SPRITE_DUST_ID_1, tik + 250 * 4);
		add_dust(c->ix, c->iy, 0.0f, SPRITE_DUST_ID_0, tik + 250 * 1);
	}
...
char icas_gloomyhaze(c_cast* c)
{
	...
	add_dust(c->ix, c->iy, 1.5f - .75f * (state), SPRITE_DUST_ID_0, tik + 250 * 4);
...
char cast_suspension(c_cast* c)
{
	...
	add_dust(c->ix, c->iy, 0, SPRITE_DUST_ID_0, tik + 250 * 1);
...
{
	ch.spra = (ch.sprb = (ch.sprb ? ch.sprb : 1));
...
char cast_tempest_mist(c_cast* c)
{
	...
	CD_DEC_SPR(125 * 4, SPR_BLOCK / (8 + 2));
...
char cast_dive(c_cast* c)
{
	{
		char flag = map.arr[1].tile.arr[p2idx(c->ix, c->iy, map.arr[1].width)]
			.arr[0]
			.flag;
		if (flag == SPRITE_FLAG_WATER)
		{
			if (c->ch->state != CH_STATE_DIVE)
			{
				ddx = c->ch->dx = c->ix;
				ddy = c->ch->dy = c->iy;

				ch_set_state(c->ch, c->ch->rot, CH_STATE_WALK);
				move_render(c->ch);

				c->ch->sprite.id = TILE_DIVE_ID;
				c->ch->state = CH_STATE_DIVE;
			}
		}
		else if (flag == SPRITE_FLAG_NONE)
		{
			if (c->ch->state == CH_STATE_DIVE)
			{
				ddx = c->ch->dx = c->ix;
				...
				ch.sprite.id = ch.spriteo;
				c->ch->state = CH_STATE_IDLE;
...
char cast_bubble(c_cast* c)
{
	...
	CD_DEC_SPR_ACT_EX(250 * 3, SPR_BLOCK / (8 + 4), 125, 100 * 3);
...
char (*castfn[])(c_cast* c) =
{
	...
	&cast_dynamic_current, &cast_spirit_element,&cast_tempest_mist, &cast_deep_reserve, &cast_elusive_gale, &cast_dive, &cast_bubble, 0,
	0, 0, 0, 0, 0, 0, 0, 0,
};
