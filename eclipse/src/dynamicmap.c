...
void init_dm()
{
	...
	appendmap(&buf, PATH_DMD);
	free_map(&buf);

	map = readmapp(PATH_DMD, 0);
	linkmap();
}
...
void ch_transition_update()
{
		...
		&& screen_mode != SCREEN_MODE_DISP_TRS)
	{
		...
		if (tik0 < tik)
		{
			...
			ch_set_state(&ch, ch.rot % 4, CH_STATE_WALK);
...
//temp code v
{
	...
	//m search, e search, npcm(obsoleted), sm search
	128, 128, 0, 128,
...
extern inline int dm_rcomp(int i, int l);
