...
const char* GlobalText[GTEXT_SIZE] =
{
	"It doesn't seem to respond.",
	"You found a XXXXXXXXXXXXXXXX",
	"Welcome! Here we surf! %%122%%121%%122",
...
void push_back_sprdataa_str(s_spritedata_a* array, const char* str)
{
	...
	for (int i = 0; i < strlen(str); i++)
	{
		if (str[i] == '%' && str[i + 1] == '%')
		{
			...
			s_spritedata* tmp = (s_spritedata*)&sprdata[buf + TEXT_OFFSET];
		...
		{
			if (str[i] == 0) c = NULL;
			else if (str[i] == 1)c = &(sprdata[+(!!1) + 4 + 134 - 28 + TEXT_OFFSET]);
			else c = &(sprdata[chtotid(str[i]) + TEXT_OFFSET]);
